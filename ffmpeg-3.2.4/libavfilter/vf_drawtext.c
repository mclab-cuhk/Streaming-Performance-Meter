...
//RUDOLF
extern int my_num_frames;
extern char my_ftypes[500000];
extern long my_fbytes[500000];
char g_text[256];
...
char dummystr[128];
float dummyfloat;
//if ((agp=fopen("../aggregate.csv","r"))!=NULL)
if ((agp=fopen("../ss_video00_A_Good_Day_to_Die_Hard.txt","r"))!=NULL)
{
 int i = 0;
 char line[128], ftype;
 int fbyte;

 while (fgets(line,128,agp))
 {
  if (strlen(line) > 5)
  {
   sscanf(line, "%s,%s,%c,%f,%d", dummystr, dummystr, &ftype, &dummyfloat, &fbyte);
   my_ftypes[i] = ftype;
   my_fbytes[i] = fbyte;
   i++;
  }
...
if (my_num_frames > 0)
{
sprintf(text, "frame %d type %c bytes %d", my_frno, my_ftypes[my_frno], my_fbytes[my_frno]);
//sprintf(g_text, "%d %c %d", my_frno, my_ftypes[my_frno], my_fbytes[my_frno]);
sprintf(g_text, "%06d", my_frno+999000);
}
    if ((len = s->expanded_text.len) > s->nb_positions) {
        if (!(s->positions = av_realloc(s->positions, len*sizeof(*s->positions))))
            return AVERROR(ENOMEM);
        s->nb_positions = len;
    }
    ...
    for (i = 0, p = text; *p; i++) {
        ...
        if (s->use_kerning && prev_glyph && glyph->code) {
            FT_Get_Kerning(s->face, prev_glyph->code, glyph->code, ft_kerning_default, &delta);
            x += delta.x >> 6;
        }
    ...
    /* draw box */
    if (s->draw_box)
        ff_blend_rectangle(&s->dc, &boxcolor, frame->data, frame->linesize, width, height, s->x - s->boxborderw, s->y - s->boxborderw, box_w + s->boxborderw * 2, box_h + s->boxborderw * 2);
...
if (1)
{
	...
	if (bc = Barcode_Create(g_text))
	{
		int i, j, barlen, printable=1;
		double f1, f2, fsav=0;
		int mode = '-'; /* text below bars */
		double scalef=1, xpos, x0, y0, yr, w0;
		char *ptr;
		char c;
		...
		{
			case 0:
				offset_width = width/2 - bcwidth -100;
				offset_height = yborder;
				break;
			case 1:
				offset_width = width/2 +100;
				offset_height = yborder;
				break;
			case 2:
				offset_width = width/2 +100;
				offset_height = yborder + bcheight;
				break;
			case 3:
				offset_width = width/2 - bcwidth -100;
				offset_height = yborder + bcheight;
        ...
        ff_blend_rectangle(&s->dc, &boxcolor, frame->data, frame->linesize, width, height, offset_width -100, offset_height -20, bcwidth +200, bcheight);

		bc->margin = 0;
		Barcode_Position(bc, bcwidth -50, bcheight -10, 0, 0, 0.0);
		Barcode_Encode(bc, BARCODE_128 | BARCODE_OUT_PS | BARCODE_OUT_NOHEADERS);
...
//fprintf(stderr, "[%5.2f %6.2f %6.2f %5.2f]\n", yr, x0, y0, (j * scalef) - 0.15);
				w0 = (j * scalef) - 0.15;
				ff_blend_rectangle(&s->dc, &bordercolor, frame->data, frame->linesize, width, height, offset_width + (x0 - w0/2)*6, offset_height +10, w0*6, yr - 40);
			}
			xpos += j * scalef;
   		}
//fprintf(stderr, "]\n");
...
if (0)
{
	...
	bool ok = qrcodegen_encodeText(g_text, tempBuffer, qrcode, errCorLvl, qrcodegen_VERSION_MIN, qrcodegen_VERSION_MAX, qrcodegen_Mask_AUTO, true);
	if (ok)
	{
		...
		{
			case 0:
				offset_width = width/2 - size*pixelSize/2 - padding;
				offset_height = yborder;
				break;
			case 1:
				offset_width = width/2 + size*pixelSize/2 + padding;
			...
			case 2:
				offset_width = width/2 - size*pixelSize/2 - padding;
				offset_height = yborder + size*pixelSize + padding;
				break;
			case 3:
				offset_width = width/2 + size*pixelSize/2 + padding;
				offset_height = yborder + size*pixelSize + padding;
		...
		for (y = -border; y < size + border; y++)
		{
			for (x = -border; x < size + border; x++)
			{
				if (qrcodegen_getModule(qrcode, x, y))
					...
					QRcolor = &boxcolor;
				ff_blend_rectangle(&s->dc, QRcolor, frame->data, frame->linesize, width, height, offset_width - size*pixelSize/2 + x*pixelSize, offset_height + y*pixelSize, pixelSize, pixelSize);
			}
		}
	}
}
...
my_frno++;
    ...
    if (s->shadowx || s->shadowy) {
        if ((ret = draw_glyphs(s, frame, width, height, &shadowcolor, s->shadowx, s->shadowy, 0)) < 0)
            return ret;
    }

    if (s->borderw) {
        if ((ret = draw_glyphs(s, frame, width, height, &bordercolor, 0, 0, s->borderw)) < 0)
            return ret;
    }
    if ((ret = draw_glyphs(s, frame, width, height, &fontcolor, 0, 0, 0)) < 0)
        return ret;
