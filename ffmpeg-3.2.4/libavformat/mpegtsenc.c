...
//RUDOLF
int my_num_frames;
char my_ftypes[500000];
long my_fbytes[500000];
...
if ((agp=fopen("../ss_video00_BLACK_BOOK.txt","r"))!=NULL)
{
 int i = 0;
 char line[128], *sp;
 ...
 while (fgets(line,128,agp))
 {
  if (strlen(line) > 5)
  {
   ...
   if (sp = strrchr(line,','))
   {
    my_fbytes[i] = atoi(sp+1);
   }
//fprintf(stderr, "%d FTYPE %c FBYTE %d\n", i, my_ftypes[i], my_fbytes[i]);
   i++;
  }
 }
 my_num_frames = i;
...
if (last_pts != pts)
{
 ...
 {
  int pd = my_fbytes[(pts-pts_base)*24/90000] /188;

  fprintf(stderr, "PTS %.4f", (double)(pts-pts_base)/90000);
  fprintf(stderr, " -PTS %d", my_fbytes[(pts-pts_base)*25/90000]);
  fprintf(stderr, " insert padding %d times %d%20s", nultime++, pd, " ");
  fprintf(stderr, "\n");
  while (--pd > 0)
  {
   mpegts_insert_null_packet(s);		// add 188 bytes padding
  }
 }
 last_pts = pts;
}
